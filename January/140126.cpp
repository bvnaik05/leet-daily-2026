...
// No idea about Segment Tree solution yet.
// Used Official Editorial solution instead.

// Dry run
// Input: squares = [[0,0,2],[1,1,2]]
// Events: [(0,1,0,2),(1,1,1,3),(2,-1,0,2),(3,-1,1,3)]
// xs = [0,1,2,3]
// Scan Steps:
// 1. y=0, delta=1, xl=0, xr=2  
//    len=0, total_area=0
//    Update Segment Tree: add [0,2)
// 2. y=1, delta=1, xl=1, xr=3  
//    len=2, total_area=2*(1-0)=2   
//    Update Segment Tree: add [1,3)
// 3. y=2, delta=-1, xl=0, xr=2
//    len=3, total_area=2+(3*(2-1))=5
//    Update Segment Tree: remove [0,2)
// 4. y=3, delta=-1, xl=1, xr=3
//    len=2, total_area=5+(2*(3-2))=7
//    Update Segment Tree: remove [1,3)
// Prefix sums: psum=[0,2,5,7]
// Widths: widths=[0,2,3,2]
// Total area = 7
// Target area = (7 + 1) / 2 = 4
// Binary Search Steps:
// i = lower_bound(psum, 4) - 1 = 1
// area = psum[1] = 2
// width = widths[1] = 2
// height = events[1].y = 1
// Result = 1 + (7 - 2*2) / (2*2.0) = 1 + 3/4 = 1.75
